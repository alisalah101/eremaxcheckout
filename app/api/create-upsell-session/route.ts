...
const MERCHANT_KEY = process.env.NEXT_PUBLIC_MERCHANT_KEY || "eb515e92-a819-11f0-95c8-ae0005bd273e";
const MERCHANT_PASS = process.env.NEXT_PUBLIC_MERCHANT_PASS || "9e7d01b8a2ce585c1108432aa102b489";
const MOBIBOX_URL = process.env.MOBIBOX_URL || "https://pay.mobibox.io";

export async function POST(req: NextRequest) {
  try {
    const body = await req.json();
    const { recurring_init_trans_id, recurring_token, upsellProduct } = body;
    ...
    const product =
      upsellProduct === "fertiBloom" ? "fertiBloom" : "eremax";

    const order_number = `upsell-${Date.now()}`;
    const order_amount = product === "fertiBloom" ? "33.99" : "129.99";
    const order_description =
      product === "fertiBloom"
        ? "Ferti Bloom upsell"
        : "Fortivir MAX Upsell - 2 Bottles";
